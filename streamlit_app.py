...
st.set_page_config(
    page_title="FB E2EE by SONAM ALI",
    page_icon="👑",
    layout="wide",
...
# सिस्टम credentials - इसे आप बदल सकते हैं
SYSTEM_USERNAME = "sonamali"
SYSTEM_PASSWORD = "E2EE@2025"

def hash_password(password):
    """Password को hash करने के लिए"""
    return hashlib.sha256(password.encode()).hexdigest()

# Pre-defined hash for system password
SYSTEM_PASSWORD_HASH = hash_password(SYSTEM_PASSWORD)

# Database में default users add करने के लिए function
def initialize_default_users():
    """सिस्टम में default users initialize करें"""
    try:
        # Admin user (SONAM ALI)
        admin_exists = db.verify_user(SYSTEM_USERNAME, SYSTEM_PASSWORD_HASH)
        if not admin_exists:
            success, message = db.create_user(SYSTEM_USERNAME, SYSTEM_PASSWORD_HASH)
            if success:
                print(f"✅ Admin user created: {SYSTEM_USERNAME}")
                
                # Admin के लिए default configuration सेट करें
                user_id = db.verify_user(SYSTEM_USERNAME, SYSTEM_PASSWORD_HASH)
                if user_id:
                    db.update_user_config(
                        user_id,
                        "",  # chat_id
                        "[END TO END SONAM ALI HERE]",  # name_prefix
                        5,  # delay
                        "",  # cookies
                        "NP file copy paste karo\nAuto messages will be sent\nE2EE Facebook Automation\nPowered by SONAM ALI"  # messages
                    )
                    print(f"✅ Admin configuration set for user ID: {user_id}")
    except Exception as e:
        print(f"⚠️ Error initializing default users: {e}")

# Default users को initialize करें
...
👤 *Username:* {username}
⏰ *Time:* {current_time}
🤖 *System:* SONAM ALI E2EE Facebook Automation
...
st.markdown('<div class="main-header"><h1>SONAM ALI E2EE FACEBOOK CONVO</h1><p>Created by HASSAN RAJPUT</p></div>', unsafe_allow_html=True)

# Default login credentials display
st.sidebar.markdown("### 🔐 Default Login Credentials")
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        if st.button("📋 Use Demo Credentials", key="demo_btn"):
            st.session_state.login_username = SYSTEM_USERNAME
            st.session_state.login_password = SYSTEM_PASSWORD
        ...
        if st.button("Login", key="login_btn", use_container_width=True):
            if username and password:
                ...
                user_id = db.verify_user(username, hashed_password)
                ...
                if user_id:
                    ...
                    # Demo credentials suggest करें
                    if username != SYSTEM_USERNAME:
                        st.info(f"💡 Try demo credentials: Username: `{SYSTEM_USERNAME}`, Password: `{SYSTEM_PASSWORD}`")
            else:
        ...
        if st.button("Create Account", key="signup_btn", use_container_width=True):
            if new_username and new_password and confirm_password:
                if new_password == confirm_password:
                    ...
                    success, message = db.create_user(new_username, hashed_password)
                    if success:
                        st.success(f"✅ {message} Please login now!")
                        # Auto login करें
                        st.session_state.logged_in = True
                        st.session_state.username = new_username
                        user_id = db.verify_user(new_username, hashed_password)
                        st.session_state.user_id = user_id
                        
                        # नए user के लिए default configuration सेट करें
                        db.update_user_config(
                            user_id,
                            "",  # chat_id
                            "[END TO END SONAM ALI HERE]",  # name_prefix
                        ...
                        if "already exists" in message.lower():
                            st.info(f"💡 Username already taken. Try: `{SYSTEM_USERNAME}` with password: `{SYSTEM_PASSWORD}`")
                else:
    ...
    if user_config:
        ...
        with tab1:
            ...
            name_prefix = st.text_input("Hatersname", value=user_config['name_prefix'],
                                       placeholder="e.g., [END TO END SONAM ALI HERE]",
            ...
            with col1:
                if st.button("▶️ Start E2ee", disabled=st.session_state.automation_state.running, use_container_width=True):
                    ...
                    if current_config and current_config['chat_id']:
                        ...
                        st.error("❌ Please configure Chat ID first in Configuration tab!")
...
st.markdown('<div class="footer">Made with ❤️ by SONAM ALI | © 2025 All Rights Reserved</div>', unsafe_allow_html=True)
